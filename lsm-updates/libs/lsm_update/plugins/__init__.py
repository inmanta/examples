...
def _validating_entity(entity_id: "string") -> "bool":
    ...
    model_state = os.environ.get(
        lsm_const.ENV_MODEL_STATE, inmanta_lsm.model.ModelState.active
    )

    return (
        entity_id == instance_id
        and model_state == inmanta_lsm.model.ModelState.candidate
    )
...
def embedded_entities_paired(
    ...
    try:
        old, new = _diff_attributes_based_on_assignment_state(
            parent_assignment, lambda x, y: (x, y)  # just get old and new
        )
        ...
        def process_item(key, old, new):
            ...
            LOGGER.info(
                "Item %s in %s updated from %s to %s",
                key,
        ...
        return [
            process_item(
                entity, old_by_key.get(entity, None), new_by_key.get(entity, None)
            )
            for entity in all_embedded_entity_keys
        ]
        ...
        return [
            {"old": None, "new": v}
            for v in parent_assignment["attributes"][relation_name]
        ]
